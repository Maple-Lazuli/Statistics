...
$$
To get the pvalue, we use `qf(f*), c-2, n - c)` which is `2*(1-pf(14.8, 4, 5))` = `r 2*(1- pf(14.8, 4, 5))`
...
### Hypothesis Tes for the Slope ($\beta_1$)
...
$$
t^* = \frac{b - \beta_1}{se(b_1)} = \frac{-3}{0.4243} = -7.086
$$
...
Since our p-value is less than our alpha of 0.05, there is evidence to reject the null hypothesis.

At a significance level of 5%, there is evidence that the population slope differs from zero.
...
$$
\large \beta \pm t_{\frac{a}{2}, n - p} \times se(\beta_1)
$$

Here, the confidence interval is:

```{r}
alpha <- 1 - 0.95 # for a 95% confidence interval.

t <- qt(1 - alpha/2, 28)
se <- 0.4243
b1 <- -3

sprintf("(%f, %f)", b1 - se*t, b1 + se*t)
```

**Interpretation:** With 95% confidence, we can say the mean sign reading distance decreases somewhere between 2.14 and 3.88 feet per each one-year increase in age.
